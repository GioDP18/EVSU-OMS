...
from evsuccOMS import models, forms
...
def context_data(request):
    ...
    context = {
        ...
        'page_name' : '',
        'page_title' : '',
        'system_name' : 'EVSU-CC Organizations Management System',
        'topbar' : True,
        'footer' : True,
    }
...
def home(request):
    ...
    context['groups'] = models.Groups.objects.all().count()
    context['active_groups'] = models.Groups.objects.filter(delete_flag = 0, status = 1).all().count()
    context['inactive_groups'] = models.Groups.objects.filter(delete_flag = 0, status = 2).all().count()
    context['admins'] = User.objects.filter(is_superuser = True).all().count()
    context['members'] = models.Members.objects.all().count()
    context['active_members'] = models.Members.objects.filter(delete_flag = 0, status = 1).all().count()
    context['inactive_members'] = models.Members.objects.filter(delete_flag = 0, status = 2).all().count()
...
def groups(request):
    context = context_data(request)
    context['page'] = 'groups'
    context['page_title'] = "Organization List"
...
def per_group(request):
    ...
    context['page_title'] = 'Member List of Organization'
    context['groups'] = models.Groups.objects.filter(delete_flag = 0, status = 1).all()
    context['members'] = {}
